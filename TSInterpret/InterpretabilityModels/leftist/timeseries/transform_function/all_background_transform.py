...
from leftist.transform import Transform
    ...
    def set_background_dataset(self, background_dataset, limit='full'):
        ...
        if limit == 'full':
            ...
            if limit > len(background_dataset):
                ...
                _background_dataset_to_use = background_dataset[np.random.choice(list(range(len(background_dataset))),limit)]
        else:
            raise ValueError('limit must be set as "full" or with a positive integer of example to draw in the background dataset')
