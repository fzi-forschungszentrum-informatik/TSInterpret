...
class LIMELearningProcess(LearningProcess):
    ...
    def __init__(self, random_state, feature_selection_method="auto", model_regressor=None, distance_metric = None):
        """
        ...
        if model_regressor is None:
            self.model_regressor = Ridge(alpha=1, fit_intercept=True,random_state=self.random_state)
        random_state_generator = check_random_state(None)
        self.neighbors_generator = LIMENeighborsGenerator(random_state_generator,distance_metric=distance_metric)
    ...
    def solve(self,neighbors,idx_label, explanation_size=None):
        ...
        used_features = self._feature_selection(neighbors,proba_label_column,_explanation_size)
        ...
        easy_model.fit(neighbors.masks[:, used_features],proba_label_column, sample_weight=neighbors.kernel_weights)

        # compute performance of explanation model on the neighbors
        prediction_score = easy_model.score(neighbors.masks[:, used_features],proba_label_column, sample_weight=neighbors.kernel_weights)

        # proba predict by the explanation for the instance to explain
        explained_instance_proba_predict = easy_model.predict(neighbors.masks[0, used_features].reshape(1, -1))

        coeff = sorted(zip(used_features, easy_model.coef_))

        if len(used_features) < neighbors.masks.shape[1]:
            coeff_null = np.array([ tuple([el,0.0]) for el in list(set(range(neighbors.masks.shape[1])) - set(used_features))])
            coeff = np.concatenate((coeff,coeff_null))

        return (np.array([el[1]  for el in np.sort(coeff,axis=0)]),
    ...
    def _feature_selection(self, neighbors, proba_label_column, explanation_size):
        ...
        if self.feature_selection_method == 'none':
            ...
            return self._ft_selection_forward_selection(neighbors, proba_label_column, explanation_size)

        elif self.feature_selection_method == 'highest_weights':
            return self._ft_selection_highest_weights(neighbors, proba_label_column, explanation_size)

        elif self.feature_selection_method == 'lasso_path':
            return self._ft_selection_lasso_path(neighbors, proba_label_column, explanation_size)
            ...
            return self._feature_selection(neighbors, proba_label_column,explanation_size)

    def _ft_selection_forward_selection(self, neighbors, proba_label_column, explanation_size):
        """
        ...
        for _ in range(min(explanation_size, neighbors.masks.shape[1])):
            ...
            for feature in range(neighbors.masks.shape[1]):
                if feature in used_features:
                    continue
                clf.fit(neighbors.masks[:, used_features + [feature]], proba_label_column,
                        sample_weight=neighbors.kernel_weights)
                score = clf.score(neighbors.masks[:, used_features + [feature]],
                                  proba_label_column,
                                  sample_weight=neighbors.kernel_weights)
                if score > max_:
                    best = feature
    ...
    def _ft_selection_lasso_path(self, neighbors, proba_label_column,explanation_size):
        ...
        Returns:
            used_features (np.ndarray): index of the features to use for building the explanation model.
        """
        weighted_neighbors_masks = ((neighbors.masks - np.average(neighbors.masks, axis=0, weights=neighbors.kernel_weights))
                             * np.sqrt(neighbors.kernel_weights[:, np.newaxis]))
        weighted_proba_label_column = ((proba_label_column - np.average(proba_label_column, weights=neighbors.kernel_weights))
    ...
    def _ft_selection_highest_weights(self, neighbors, proba_label_column,explanation_size):
        """
        ...
        clf.fit(neighbors.masks, proba_label_column, sample_weight=neighbors.kernel_weights)
        feature_weights = sorted(zip(range(neighbors.masks.shape[0]),
                                     clf.coef_ * neighbors.masks[0]),
